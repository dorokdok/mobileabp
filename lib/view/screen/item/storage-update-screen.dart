...
import '../../models/item-detail_view_model.dart';
import '../../models/item_view_model.dart';
import 'package:provider/provider.dart';

class ItemStorageUpdateScreen extends StatefulWidget {
  const ItemStorageUpdateScreen({Key? key, required this.index})
      : super(key: key);

  final int index;

  @override
  State<ItemStorageUpdateScreen> createState() =>
      _ItemStorageUpdateScreenState();
}
...
class _ItemStorageUpdateScreenState extends State<ItemStorageUpdateScreen> {
  ...
  void initState() {
    ...
    stock_input.text =
        view_model.item?.locations?.elementAt(widget.index).stock.toString() ??
            '0';
  }
                  ...
                  onPressed: () {
                    if (!view_model.is_loading) {
                      Map data = {
                        ...
                        'warehouse_id': view_model.item?.locations
                                ?.elementAt(widget.index)
                                .storage
                                ?.aisle
                                ?.warehouseId ??
                            '',
                        'storage_id': view_model.item?.locations
                                ?.elementAt(widget.index)
                                .storage
                                ?.id ??
                            '',
